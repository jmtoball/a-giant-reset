...
export default async function MapView({ gpxUrl }: { gpxUrl?: string }) {
  ...
  if (!gpxUrl) return;
  const gpxXML = await (
    await fetch(gpxUrl!.startsWith('//') ? `https:${gpxUrl!}` : gpxUrl!)
  ).text();
        ...
        >
          <span className={capsFont.className}>
            ...
            <span className="highlight">
              {Math.ceil(trackData.distance / 1000)}km
            </span>
          </span>
          <span className={capsFont.className}>
            Duration:{' '}
            <span className="highlight">
              {Math.floor(trackData.duration / 60 / 60)}:
              {Math.ceil((trackData.duration / 60) % 60)}
            </span>
          </span>
          <span className={capsFont.className}>
            Elevation:&nbsp;
            <span className="highlight">
              +
              {trackData.elevationGain
                ? Math.ceil(trackData.elevationGain)
                : 'N/A'}
              m -
              {trackData.elevationLoss
                ? Math.ceil(trackData.elevationLoss)
                : 'N/A'}
              m
            </span>
          </span>
